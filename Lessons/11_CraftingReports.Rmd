...
* Easier for organizting thoughts and separating context from script
* Tidier way to share code and results with others
* Easier user interface (*mostly*)
* No need to constantly **re-run** code from the beginning
* *Helpful* To create a line break, add new line space in between
* *Helpful* To find the guide: Help >> Markdown Quick Guide (file/edit/code bar, not right hand side Help tab)
...
Dataset Names | Information
--------------|-------------
EPAair_O3_NC2018_raw|2018 EPA air samples of ozone levels in North Carolina
NTL-LTER_Lake_Carbon_Raw|Water data on dissolved organic and inorganic carbon, particulate organic matter, CO2, etc.
NWIS_SiteFlowData_NE_Raw|Water gage heights at different sample sites
...
# FOR EACH LAKE -- ASK FOR HELP

#Summary Stats (mean, minimum, maximum, and standard deviation): total nitrogen
...
#Summary Stats: total phosphorus 
...
kable(nutrients_sum_tn,
      col.names = c('Mean', 'Min', 'Max', 'Standard Deviation'),
      caption = "Total Nitrogen Summary Statistics")

kable(nutrients_sum_tp,
      col.names = c('Mean', 'Min', 'Max', 'Standard Deviation'),
      caption = "Total Phosphorous Summary Statistics")

```

```{r plotN, echo=FALSE, fig.align = 'left', fig.cap = "Total Nitrogen by Lake Over Time"}

#basic time series
TNplot <-
    ggplot(nutrients_processed, aes(x = sampledate, y = tn_ug, color=lakename)) +
    geom_line() +
    ylab(expression("Total Nitrogen (ug/L)")) +
    xlab(expression("Date")) +
    labs(color="Lake Name")

#time series split by lake
TNplot_wrap <-
    ggplot(nutrients_processed, aes(x = sampledate, y = tn_ug, color=lakename)) +
    geom_line() +
    facet_wrap(vars(lakename), nrow = 4) +
    ylab(expression("Total Nitrogen (ug/L)")) +
    xlab(expression("Date")) +
    labs(color="Lake Name")

#time series split by lake and date to see unique trends by year
nutrients_month <-
    nutrients_processed %>%
    mutate(month = month(sampledate))

TNplot_wrapyear <-
    ggplot(nutrients_month, aes(x = month, y = tn_ug, color=lakename)) +
    geom_line() +
...
print(TNplot)
print(TNplot_wrap)
print(TNplot_wrapyear)

```

```{r plotP, echo=FALSE, fig.align = 'left', fig.cap = "Total Phosphorous by Lake Over Time"}

#time series
TPplot <-
    ggplot(nutrients_processed, aes(x = sampledate, y = tp_ug, color=lakename)) +
    geom_line() +
    ylab(expression("Total Phosphorous (ug/L)")) +
    xlab(expression("Date")) +
    labs(color="Lake Name")
...
#time series split by lake and date to see unique trends by year
TPplot_wrapyear <-
    ggplot(nutrients_month, aes(x = month, y = tp_ug, color=lakename)) +
    geom_line() +
    facet_grid(lakename ~ year4) +
    theme(strip.text.y = element_blank()) +
    ylab(expression("Total Phosphorous (ug/L)")) +
...
> The first set of basic time series graphs illustrates that total nitrogen seems to be less variable over time than total phosphorous. This pattern is further emphasized in the facet grid graphs where it visualizes the pattern for each lake by year: with few exceptions (such as 1996 and West Long Lake) nitrogen levels remain fairly constant throughout the year with slight elevations near the end of summer; in contrast, phosphorous levels show a variety of temporal trends depending on their year and lake, however it does appear that its levels increase tend to increase with warmer temperatures (later in the summer). Lastly, the facet wrap graphs demonstrate that East Long, West Long, and Peter Lakes experience significantly more variability in nitrogren and phosphorous over the years than the others, although it is worth noting that there is insufficient data samples to make this assessment of Crampton and Hummingbird Lakes.   
----
The next steps in analyzing this dataset should include corresponding temperature and gage heights to answer whether weather and storm events can explain some of these trends. Specifically, running regressions to identify correlations, establish an ordinary least squares best fit line, and test for joint correlations can be conducted to answer these questions. This output will also indicate the strength of our model and indicate whether there are omitted variables that we have not considered. 
