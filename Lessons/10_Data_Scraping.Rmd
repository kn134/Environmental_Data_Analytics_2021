...
df_withdrawals <- data.frame("Month" = rep(1:12),       #sequence of numbers 
                             "Year" = rep(2000,12),     #make 12 instances of 2000 (length of table)
...
df_withdrawals <- df_withdrawals %>% 
  mutate(Registrant = !!the_registrant,   #don't set it as a string, it refers as a variable
...
scrape.it <- function(the_year, the_facility){
  ...
  #Sys.sleep(5) #uncomment this if you are doing bulk scraping! - makes code run more slowly ex for iterations so you don't bog down the server (denial service attack) - 5 seconds ex
...
ggplot(df_2020,aes(y = Avg_Withdrawals_mgd, x=as.factor(Month))) +
  geom_boxplot()
