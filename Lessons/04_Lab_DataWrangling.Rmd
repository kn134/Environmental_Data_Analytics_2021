...
NTL.phys.data.JunethruOctober <- filter(NTL.phys.data, daynum %in% c(152:304)) #rows for June to Oct #%in% finds within those 
...
NTL.phys.data.TuesDates <- 
  NTL.phys.data %>% # means what comes up next/then
  filter(year4 %in% c(1990:1999)) %>%
  filter(lakename == "Tuesday Lake")

# same output; combined script
NTL.phys.data.TuesDates2 <- 
  NTL.phys.data %>% # means what comes up next/then
...
NTL.phys.data.processed <- 
  ...
  mutate(temperature_F = (temperature_C*9/5) + 32) #convert to F and create new column called temperature_F


#Exercise 3: Using a pipe filter NTL.phys.data for Tuesday Lake from 1990 through 1999 only for July 

### Q: go back to understand this
Tues1990s <-
  NTL.phys.data %>%
  filter(lakename == "Tuesday Lake") %>%
  filter(year4 %in% c(1990:1999)) %>%
...
#Exercise 4: Using the data from part 3, a pipe and the summarise() function find the mean surface temperature (hints: you will need to add another filter for depth==0). Make sure you eliminate NAs before computing the means
  ...
  # na.omit omits but doesn't remove
  # need to use drop_NA
...
MeanSurfTemp <-
  ...
  summarise(mean = (mean(temperature_C))) # mean = is the name; mean(temp) is the math
...
# longer because transform into rows (makes dataset longer)
# water and deck columns
# name new columns 
...
## make shorter, wider -- break row into column; depth row values into depth column as column names

NTL.spread <- pivot_wider(NTL.phys.data,names_from = "depth", values_from = "temperature_C")
