...
P.P.surface <- PeterPaul.chem.nutrients %>%
  filter(depth == 0)

Temp.plot <- ggplot(P.P.surface, aes(x = daynum, y = temperature_C, color = year4)) +
  geom_point() +
...
P.P.temperature <- ggplot(PeterPaul.chem.nutrients, aes(x=sampledate, y=temperature_C, color = depth)) + geom_point(size=.5) + scale_color_viridis(option = "magma", direction = -1)

print(P.P.temperature)
  
  ### see if changing dot size is different when it's within geom_point (hint: probably)
...
plot.aqi = ggplot(PeterPaul.chem.nutrients, aes(x=))
